...
(function() {
  ...
  describe("utils", function() {
    ...
    return describe('generateAkamaiToken', function() {
      beforeEach(function() {
        return cloudinary.config({
          akamai_key: '00112233FF99'
        });
      });
      afterEach(function() {
        return cloudinary.config(true);
      });
      it("should generate an Akamai token with start_time and window", function() {
        var token;
        token = utils.generateAkamaiToken({
          start_time: 1111111111,
          acl: '/image/*',
          window: 300
        });
        return expect(token).to.eql('__cld_token__=st=1111111111~exp=1111111411~acl=/image/*~hmac=0854e8b6b6a46471a80b2dc28c69bd352d977a67d031755cc6f3486c121b43af');
      });
      it("should generate an Akamai token with window", function() {
        var expiration, first_exp, match, second_exp, token;
        ...
        return expect(utils.generateAkamaiToken({
          acl: '*',
          end_time: expiration
        })).to.eql(token);
      });
      it("should accept a key", function() {
        return expect(utils.generateAkamaiToken({
          acl: '*',
          end_time: 10000000,
          key: '00aabbff'
      ...
      return it("should throw if no end_time or window is provided", function() {
        return expect(function() {
          return utils.generateAkamaiToken({
            acl: '*'
          });
        }).to.throwError();
      });
